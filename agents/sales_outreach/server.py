"""Sales Outreach Agent — A2A HTTP server.

Wraps SalesOutreachAgent behind:
  GET  /.well-known/agent.json   A2A discovery
  POST /tasks                    {"question": "...", "context": "..."?}
                                 → {"agent", "question", "answer",
                                    "status", "error"}

Identity flow is identical to the other agents: caller's JWT in the
``Authorization: Bearer`` header → verified → forwarded into the
outreach MCP tool call so RLS enforces territory rules on submit_draft.
"""
...
observability.setup("sales_outreach_agent")

from fastapi import FastAPI, Request as FastAPIRequest  # noqa: E402
from pydantic import BaseModel, Field  # noqa: E402
import uvicorn  # noqa: E402

from agents.sales_outreach.agent import SalesOutreachAgent  # noqa: E402
...
from config import (  # noqa: E402
    ...
    MODEL_NAME,
    OLLAMA_BASE_URL,
    SALES_OUTREACH_AGENT_PORT,
)


AGENT_KEY = "sales_outreach"
CARD_PATH = os.path.join(_THIS_DIR, "agent_card.json")
PORT = SALES_OUTREACH_AGENT_PORT
...
_agent: Optional[SalesOutreachAgent] = None
...
async def run_task(req: TaskRequest, request: FastAPIRequest) -> TaskResponse:
    if _agent is None:
        return TaskResponse(
            agent=AGENT_NAME, question=req.question, answer="",
            status="failed", error="Agent is not initialised",
        )
    ...
    async with _inference_lock:
        try:
            ...
            return TaskResponse(
                agent=AGENT_NAME, question=req.question,
                answer=answer or "", status="completed",
            )
        except Exception as e:
            return TaskResponse(
                agent=AGENT_NAME, question=req.question, answer="",
