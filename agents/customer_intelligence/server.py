"""Customer Intelligence Agent — A2A HTTP server.

Wraps CustomerIntelligenceAgent behind two endpoints:
...
observability.setup("customer_intelligence_agent")

from fastapi import FastAPI, Request as FastAPIRequest  # noqa: E402
from pydantic import BaseModel, Field  # noqa: E402
import uvicorn  # noqa: E402

from agents.customer_intelligence.agent import CustomerIntelligenceAgent  # noqa: E402
from auth import extract_bearer, verify_jwt  # noqa: E402
from config import (  # noqa: E402
    AGENTS,
    CUSTOMER_INTELLIGENCE_AGENT_PORT,
    LLM_PROVIDER,
    MCP_SERVERS,
    MCP_TRANSPORT,
    MODEL_NAME,
...
AGENT_KEY = "customer_intelligence"
CARD_PATH = os.path.join(_THIS_DIR, "agent_card.json")
PORT = CUSTOMER_INTELLIGENCE_AGENT_PORT
...
_agent: Optional[CustomerIntelligenceAgent] = None
...
async def lifespan(app: FastAPI):
    global _agent
    _preflight()
    _agent = CustomerIntelligenceAgent()
    await _agent.start()
    try:
        yield
    finally:
...
async def run_task(req: TaskRequest, request: FastAPIRequest) -> TaskResponse:
    """Run the agent, forwarding the caller's JWT into MCP tool calls."""
    if _agent is None:
        return TaskResponse(
