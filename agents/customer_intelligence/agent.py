"""Customer Intelligence Agent — domain specialist over customer data.

Connects to three MCP servers: ``database``, ``scoring``, ``report``.
The list of servers is read directly from the AGENTS registry in
config.py so it stays in sync with the declarative project layout.

All arithmetic (churn risk, LTV, segmentation, cohorts) is delegated
to the scoring server — the agent never computes scores itself.
...
Built on ``langgraph.prebuilt.create_react_agent`` — same pattern as
the data_analysis and sales_intelligence agents.
"""
...
PROMPT_VERSION = "v1"
SYSTEM_PROMPT = load_prompt("customer_intelligence", PROMPT_VERSION)
...
def _build_mcp_client_config(
    ...
    identify the caller for RLS enforcement. stdio cannot carry headers.
    """
...
class CustomerIntelligenceAgent:
    ...
    async def __aenter__(self) -> "CustomerIntelligenceAgent":
        await self.start()
        return self
