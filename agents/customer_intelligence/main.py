"""Customer Intelligence Agent — entry point.
...
consumes is reachable before launching the agent. Then either answers
a single question (CLI arg) or drops into an interactive REPL.

Usage:
    # From the enterprise-agents project root:
    python agents/customer_intelligence/main.py
    python agents/customer_intelligence/main.py "Who is at churn risk?"
...
from agents.customer_intelligence.agent import (  # noqa: E402
    AGENT_NAME,
...
def _banner() -> None:
    print("=" * 70)
    print("  Customer Intelligence Agent — LangGraph + Ollama + MCP")
    print(f"  Transport: {MCP_TRANSPORT.upper()}")
    print("=" * 70)
    print("  I specialise in customer behaviour, churn risk, and segmentation.")
    print()
    print("  Example questions:")
    print("    1. Who are our top 5 customers by lifetime value?")
    print("    2. Which customers are at critical churn risk?")
    print("    3. How has retention changed by signup cohort?")
    print("    4. Write a full intelligence report on the customer base.")
    print()
...
async def _run_once(question: str) -> None:
    try:
        async with CustomerIntelligenceAgent() as agent:
